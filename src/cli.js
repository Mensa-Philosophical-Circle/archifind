...
function ensureUiIsBuilt({ autoBuild, forceRebuild }) {
  ...
  if (!fs.existsSync(uiNodeModulesPath)) {
    console.info('[archifind] Installing UI dependencies...');
    runCommand('npm', ['install', '--no-audit', '--no-fund'], uiDir);
  }

  console.info('[archifind] Building UI...');
...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.info('[archifind] Install steps');
      installSteps.forEach((step) => console.info(step));
      console.info('');
      console.info('[archifind] Shell snippet');
      console.info(snippet);

      if (options.write) {
        const result = writeProfileSnippet(profilePath, snippet);
        console.info('');
        console.info(`[archifind] Wrote setup snippet to ${profilePath}`);
        if (result.created) {
          console.info('[archifind] Created a new shell profile file.');
        }
        console.info('[archifind] Reload your shell after this change.');
      }
    } catch (error) {
      console.error(error);
      process.exitCode = 1;
...
program
  ...
  .action(async (dir, options) => {
    try {
      ...
      if (!includeEnv && fs.existsSync(targetDir) && fs.lstatSync(targetDir).isDirectory()) {
        const envFiles = fs
