...
function buildFallbackAnswer(question, graph, selectedNodeId) {
  ...
  const topGroups = Array.isArray(graph?.architectureGroups)
    ...
    : [];
  const groupSummary =
    topGroups.map((group) => `${group.id} (${group.count})`).join(", ") ||
    "no groups";
...
async function askHuggingFace(prompt) {
  ...
  try {
    ...
    console.warn(
      "[archifind] AI chat request failed:",
      error?.message ?? error,
    );
    return null;
  } finally {
...
export async function startServer(targetDir, port) {
  ...
  const refreshAllKnownModes = async () => {
    const modes =
      graphCacheByMode.size > 0
        ? Array.from(graphCacheByMode.keys())
        : [DEFAULT_MODE];
  ...
  app.post("/api/graph/refresh", async (req, res) => {
    try {
      ...
      const aiNative =
        req.body?.aiNative === true || req.query.aiNative === "true";
  ...
  app.post("/api/ask", async (req, res) => {
    try {
      ...
      const answer =
        aiAnswer || buildFallbackAnswer(question, graph, selectedNodeId);
  ...
  app.use((req, res, next) => {
    ...
    res
      .status(200)
      .send(
        'API is running. UI needs to be built with "npm run build" in the ui directory.',
      );
  });
