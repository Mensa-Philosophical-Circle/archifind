...
function buildGraphSummary(graph, selectedNodeId) {
  ...
  const topGroups = Array.isArray(graph?.architectureGroups) ? graph.architectureGroups.slice(0, 10) : [];
  const topNodes = Array.isArray(graph?.nodes) ? graph.nodes.slice(0, 18) : [];
  const relatedEdges = selectedNode
    ? (graph?.edges ?? []).filter(edge => edge.source === selectedNode.id || edge.target === selectedNode.id).slice(0, 12)
    : [];
...
function buildFallbackAnswer(question, graph, selectedNodeId) {
  const selectedNode = selectedNodeId ? graph?.nodes?.find(node => node.id === selectedNodeId) ?? null : null;
  const topGroups = Array.isArray(graph?.architectureGroups) ? graph.architectureGroups.slice(0, 3) : [];
  const groupSummary = topGroups.map(group => `${group.id} (${group.count})`).join(', ') || 'no groups';
  ...
  return [
    ...
    selectedNode ? `You selected ${selectedNode.data?.label ?? selectedNode.id}, which is classified as ${selectedNode.data?.role ?? 'unknown'}.` : null,
